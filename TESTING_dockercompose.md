...
docker run --rm --net demo-polysite-drupal_default amazeeio/dockerize dockerize -wait tcp://mariadb:3306 -timeout 1m
```
...
docker ps --filter label=com.docker.compose.project=demo-polysite-drupal | grep Up | grep demo-polysite-drupal-nginx-1
docker ps --filter label=com.docker.compose.project=demo-polysite-drupal | grep Up | grep demo-polysite-drupal-mariadb-1
docker ps --filter label=com.docker.compose.project=demo-polysite-drupal | grep Up | grep demo-polysite-drupal-php-1
docker ps --filter label=com.docker.compose.project=demo-polysite-drupal | grep Up | grep demo-polysite-drupal-cli-1

# Should ssh against the cli container by default
docker compose exec -T cli bash -c "env | grep LAGOON=" | grep cli-drupal

# Should have the correct environment set
docker compose exec -T cli bash -c "env" | grep LAGOON_ROUTE | grep demo-polysite-drupal.docker.amazee.io
